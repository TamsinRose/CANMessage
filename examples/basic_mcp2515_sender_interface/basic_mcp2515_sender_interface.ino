...
// Define the I/O pin used for C/S of the CAN bus interface
const int SPI_CS_PIN = 9;
// Establish a CAN bus interface, in this case using the mcp2515 library 
mcp2515_can CANinterface(SPI_CS_PIN);
CANMessageSenderMCP2515 CANSender(CANinterface);

// A 5 byte CAN frame, with CAN ID 0x0B5, to be sent every 20ms on the mcp2515_can CANinterface
CANMessage<5> CAN_0B5(0x0B5, 20, CANSender);
// An 8 byte CAN frame, with CAN ID 0x0B8, to be sent every 100ms on the mcp2515_can CANinterface. 
